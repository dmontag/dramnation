...
addDistillery
  = "add"i WS "distillery"i WS name:qString mods:(WS distilleryModifier)* { 
...
caskWoodType
  ...
  / bourbonWood
sherryBourbonWood
  = "sherry/bourbon"i { return "sherryBourbon"; }
oakBourbonWood
  = "oak/bourbon"i { return "oakBourbon"; }
oakWood
  = "oak"i { return "oak"; }
sherryWood
...
findTastingNote
  = "find"i WS "tasting"i WS "note"i WS "containing"i WS q:qString { return {operation:"find", kind:"tastingNote", phrase:q}; }

findDistillery
  = "find"i WS "distillery"i region:(WS "in"i WS qString) ? WS "with"i WS whisky WS "where"i WS ast:orExpression 
  { return {operation:"find", kind:"distillery", region:(region?region[3]:null), ast:ast}; }
...
notExpression
  = "not"i WS exp:atomExpression { return {kind:"not", input:exp}; }
realAtomExpression
  = unaryExpression
  / operatorExpression
unaryExpression
  = literal:( 
    ...
    / whiskyOriginalBottling
    / ownedExpression
  ) { return {kind:"is", input:literal[0]}; }
ownedExpression
  = "owned"i { return ["owned"]; }
...
operatorExpression
  ...
  / finishExpression
  / bottlerExpression
  / distilleryClosedExpression
  / distilleryExpression
  / regionExpression
  / tastingNoteExpression
percentageExpression
...
distilleryExpression
  = "distilled"i WS "by"i WS d:qString { return {kind:"distillery", input:["name", d]}; }
regionExpression
  = "region"i WS r:qString { return {kind:"region", input:r}; }
distilleryClosedExpression
  = "distillery"i WS "closed"i { return {kind:"distillery", input:["closed", true]}; }
gtLtEq
  = ">" / "<" / "="
tastingNoteExpression
  = "note"i WS "contains"i WS q:qString { return {kind:"note", input:q}; }
